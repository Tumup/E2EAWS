...
#Eliminar tipos de datos especificos de DynamoDB
...
def lambda_handler(event, context):
    ...
    for record in event['Records']:
        try:
            ...
            file_name = f"videojuego_{transformed_data['id']}_{datetime.now().strftime('%Y%m%d%H%M%S')}.json"
            
            # Guardar los datos transformados en un archivo en S3
            s3.put_object(
                Bucket='aws-project-videojuegos',
                Key=file_name,
